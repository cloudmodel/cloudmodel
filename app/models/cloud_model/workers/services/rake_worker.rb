    ...
    module Services
      class RakeWorker < CloudModel::Workers::Services::BaseWorker
        ...
        def auto_start_service
          ...
          @host.exec "ln -sf /etc/systemd/system/rake-#{@model.rake_task}.service #{@guest.deploy_path.shellescape}/etc/systemd/system/multi-user.target.wants/"
          ...
          if @model.rake_restart_on_touch
            ...
            @host.exec "ln -sf /etc/systemd/system/rake-restart-#{@model.rake_task}.path #{@guest.deploy_path.shellescape}/etc/systemd/system/multi-user.target.wants/"
            @host.exec "chown -R 100000:100000 #{@guest.deploy_path}/etc/systemd/system/rake-restart-#{@model.rake_task}.path #{@guest.deploy_path}/etc/systemd/system/rake-restart-#{@model.rake_task}.service"
          end

          @host.exec "chown -R 100000:100000 #{@guest.deploy_path}/etc/systemd/system/rake-#{@model.rake_task}.service #{@guest.deploy_path.shellescape}/etc/systemd/system/multi-user.target.wants/rake-#{@model.rake_task}.service"
        end
