...
describe CloudModel::MongodbReplicationSet do
  ...
  describe 'backup_exclude_collection_prefixes' do
    it 'uses the web image prefixes when a web image is set' do
      ...
      subject.web_image = CloudModel::WebImage.new(mongodb_backup_exclude_collection_prefixes: %w(fs search_journal))
      expect(subject.backup_exclude_collection_prefixes).to eq %w(fs search_journal)
    end
