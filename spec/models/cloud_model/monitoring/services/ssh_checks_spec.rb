...
describe CloudModel::Monitoring::Services::SshChecks do
  let(:host) { double CloudModel::Host, name: 'testhost' }
  let(:guest) { double CloudModel::Guest, host: host }
  let(:service) { double CloudModel::Services::Ssh, guest: guest }
  subject { CloudModel::Monitoring::Services::SshChecks.new service, skip_header: true }
