...
describe CloudModel::Monitoring::Services::PhpfpmChecks do
  let(:host) { double CloudModel::Host, name: 'testhost' }
  let(:guest) { double CloudModel::Guest, host: host }
  let(:service) { double CloudModel::Services::Phpfpm, guest: guest }
  subject { CloudModel::Monitoring::Services::PhpfpmChecks.new service, skip_header: true }
