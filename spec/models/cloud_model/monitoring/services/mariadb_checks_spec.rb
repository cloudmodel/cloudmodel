...
describe CloudModel::Monitoring::Services::MariadbChecks do
  let(:host) { double CloudModel::Host, name: 'testhost' }
  let(:guest) { double CloudModel::Guest, host: host }
  let(:service) { double CloudModel::Services::Mariadb, guest: guest }
  subject { CloudModel::Monitoring::Services::MariadbChecks.new service, skip_header: true }
