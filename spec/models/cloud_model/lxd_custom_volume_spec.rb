...
describe CloudModel::LxdCustomVolume do
  ...
  describe 'last_backup_at' do
    ...
    it 'should return the time of the newest backup snapshot' do
      allow(subject).to receive(:zfs_backup_snapshots).and_return(["#{dataset}@cm-bkp-20200403133742"])
  ...
  describe 'backup' do
    ...
    it 'should snapshot, send full and prune on first run' do
      ...
      expect(host).to receive(:exec).with(/\Azfs snapshot #{dataset}@cm-bkp-[0-9]{14}\z/).and_return([true, ''])
      expect(subject).to receive(:send_to_backup_host).with(/#{dataset}@cm-bkp-/, nil, %r{\Adata/admin-backups/zfs_backups/}).and_return(true)
