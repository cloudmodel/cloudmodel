# encoding: UTF-8

require 'spec_helper'

describe CloudModel::Components::FusekiComponent do
  it { expect(subject).to be_a CloudModel::Components::FusekiComponent }

  describe 'base_name' do
    it 'should return "fuseki"' do
      expect(subject.base_name).to eq 'fuseki'
    end

    it 'should return "fuseki" if version is set' do
      subject.version = "42.23"
      expect(subject.base_name).to eq 'fuseki'
    end
  end

  describe 'name' do
    it 'should return :fuseki' do
      expect(subject.name).to eq :fuseki
    end

    it 'should return :fuseki@42.23 if version is set to 42.23' do
      subject.version = "42.23"
      expect(subject.name).to eq :'fuseki@42.23'
    end
  end

  describe 'human_name' do
    it 'should return "Fuseki"' do
      expect(subject.human_name).to eq "Fuseki"
    end

    it 'should return "Fuseki 42.23" if version is set to 42.23' do
      subject.version = "42.23"
      expect(subject.human_name).to eq "Fuseki 42.23"
    end
  end

  describe 'worker' do
    it 'should return worker instance' do
      host = double CloudModel::Host
      worker_class = double CloudModel::Workers::Components::FusekiComponentWorker

      expect(CloudModel::Workers::Components::FusekiComponentWorker).to receive(:new).with(host, component: subject).and_return worker_class
      expect(subject.worker host).to eq worker_class
    end
  end

  describe 'requirements' do
    it 'should require :java' do
      expect(subject.requirements).to eq [:java]
    end
  end
end