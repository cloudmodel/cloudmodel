...
describe CloudModel::Workers::Components::NginxComponentWorker do
  ...
  describe 'build' do
    ...
    it 'should apt-get nginx, passenger, certbot for ubuntu-22.04/Debian 12' do
      ...
      expect(subject).to receive(:chroot!).with('/tmp/build', "apt-get install nginx-core libnginx-mod-http-headers-more-filter libnginx-mod-http-passenger certbot python3-certbot-nginx python3-venv python3-pip -y", "Failed to install nginx+passenger+certbot+python3").ordered
