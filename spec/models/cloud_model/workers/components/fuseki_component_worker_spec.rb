...
describe CloudModel::Workers::Components::FusekiComponentWorker do
  ...
  describe 'build' do
    let(:ftp) { double login:true, chdir:true, list: ['fuseki.tar.gz'], pwd: '/apache/jena/binaries', close: true }
    before do
    ...
    it 'should get latest fuseki' do
      expect(Net::FTP).to receive(:new).with('ftp.halifax.rwth-aachen.de').and_return ftp
      expect(ftp).to receive(:login)
      expect(ftp).to receive(:chdir).with 'apache/jena/binaries/'
      ...
      expect(subject).to receive(:chroot!).with(
        '/tmp/build',
        'cd /opt && ' +
        'wget -q ftp://ftp.halifax.rwth-aachen.de/apache/jena/binaries/apache-jena-fuseki-42.23.0.tar.gz && '+
